...
uses
  ...
  RxCurrEdit, Vcl.StdCtrls, Vcl.Imaging.pngimage, Vcl.ExtCtrls;
...
begin
  ...
  begin
    ...
    begin
      ...
      Application.MessageBox('Produto atualizado?', 'Atenção', mb_iconquestion);

    end;
  end
  else
    Application.MessageBox('Não atualizado?', 'Atenção', mb_iconquestion);
...
begin
  if Application.MessageBox('Deseja deletar?', 'Atenção',
    mb_yesno + mb_iconquestion) = id_yes then
  begin
    ...
    begin
      ...
      Application.MessageBox('Produto apagado!', 'Atenção', mb_iconquestion)
    end;
  end
  else
    Application.MessageBox('Produto não apagado?', 'Atenção', mb_iconquestion);
